...
def filter_images_with_no_sem_segs(dataset_dicts):
    """
    Filter out images with no semantic segmentation ground truth masks
    (i.e., images without sem_seg annotations).
    A common training-time preprocessing on COCO dataset.
    ...
    dataset_dicts = [x for x in dataset_dicts if valid(x["sem_seg_bin_masks"])]
    num_after = len(dataset_dicts)
    logger = logging.getLogger(__name__)
    logger.info(
        "Removed {} images with no usable annotations. {} images left.".format(
...
def get_detection_dataset_dicts(
    ...
    has_sem_seg_bin_mask = "sem_seg_bin_masks" in dataset_dicts[0] 

    if filter_empty and has_instances:
        dataset_dicts = filter_images_with_only_crowd_annotations(dataset_dicts)
    if filter_empty and has_sem_seg_bin_mask:
        dataset_dicts = filter_images_with_no_sem_segs(dataset_dicts)
