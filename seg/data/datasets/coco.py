...
def register_sem_seg_instances(name, metadata, json_file, image_root, label_root):
    assert isinstance(name, str), name
    assert isinstance(json_file, (str, os.PathLike)), json_file
    assert isinstance(image_root, (str, os.PathLike)), image_root
    assert isinstance(label_root, (str, os.PathLike)), label_root
    # 1. register a function which returns dicts
    DatasetCatalog.register(name, lambda: load_sem_seg(label_root, image_root))

    # 2. Optionally, add metadata about this dataset,
    # since they might be useful in evaluation, visualization or logging
    MetadataCatalog.get(name).set(
        json_file=json_file, image_root=image_root, label_root=label_root, evaluator_type="sem_seg", **metadata
    )
