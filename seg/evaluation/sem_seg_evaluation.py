...
class SemSegEvaluator(DatasetEvaluator):
    ...
    def __init__(
        ...
        *,
        ...
        for dataset_record in DatasetCatalog.get(dataset_name):
            if dataset_record.get("sem_seg_bin_masks"):
                self.gt_type = "binary_mask"
                gt_file = dataset_record["sem_seg_bin_masks"]
            else:
                self.gt_type = "file"
                gt_file = dataset_record["sem_seg_file_name"]
            
            self.input_file_to_gt_file[dataset_record["file_name"]] = gt_file
        ...
        self._compute_boundary_iou = False  #True
    ...
    def process(self, inputs, outputs):
        ...
        for input, output in zip(inputs, outputs):
            ...
            gt = self.input_file_to_gt_file[input["file_name"]]
            if self.gt_type == "file":
                gt = self.sem_seg_loading_fn(gt, dtype=int)
            elif self.gt_type == "binary_mask":
    ...
    def evaluate(self):
        ...
        * Dice score (dice)
        ...
        dice_valid = np.logical_and(pos_gt > 0, pos_pred > 0)
        dice[dice_valid] = (2 * tp[dice_valid]) / (pos_gt[dice_valid] + pos_pred[dice_valid])
