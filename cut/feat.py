...
class ViTFeat(nn.Module):
    ...
    def __init__(self, vit_arch='base', vit_feat='k', patch_size=16, vit_model='dino', pretrained_dir=None):
        ...
        if self.vit_model == 'dino':
            if vit_arch == 'small':
                ...
                self.feat_dim = 768
        elif self.vit_model == 'dinov2':
            self.patch_size = 14
            if vit_arch == 'small':
                model = torch.hub.load(dinov2_dir, "dinov2_vits14")
                self.feat_dim = 384
            elif vit_arch == 'base':
                model = torch.hub.load(dinov2_dir, "dinov2_vitb14")
                self.feat_dim = 768
            elif vit_arch == 'large':
                model = torch.hub.load(dinov2_dir, "dinov2_vitl14")
                self.feat_dim = 1024
            elif vit_arch == 'giant':
                model = torch.hub.load(dinov2_dir, "dinov2_vitg14")
                self.feat_dim = 1536
        elif self.vit_model == 'sammed':
            sys.path.append('../third_party')
            import importlib  
            sam = importlib.import_module("SAM-Med2D.segment_anything")
            sam_model_registry = sam.sam_model_registry
            ...
            args.sam_checkpoint = "pretrain/sam-med2d_b.pth"
            model = sam_model_registry["vit_b"](args).to(device)
            model = model.image_encoder
        elif self.vit_model == 'sam':
            from segment_anything import sam_model_registry
            if vit_arch == 'base':
                model = sam_model_registry["vit_b"](checkpoint="pretrain/sam_vit_b_01ec64.pth").image_encoder
            elif vit_arch == 'large':
                model = sam_model_registry["vit_l"](checkpoint="pretrain/sam_vit_l_0b3195.pth").image_encoder
            elif vit_arch == 'huge':
                model = sam_model_registry["vit_h"](checkpoint="pretrain/sam_vit_h_4b8939.pth").image_encoder
        elif self.vit_model == 'pretrain':
            if vit_arch == 'small':
                if self.patch_size == 16:
                    model = torch.hub.load(dino_dir, "dino_vits16")
                elif self.patch_size == 8:
    ...
    def forward(self, img):
        if self.vit_model in ['sammed', 'sam']:
...
if __name__ == "__main__":
    ...
    model = ViTFeat(vit_arch, vit_feat, patch_size, vit_model)
    img = torch.FloatTensor(1, 3, 128, 128)
